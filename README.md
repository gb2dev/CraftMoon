...
2.
  - Windows: Copy `target/debug/mod_io.dll` to `addons/modio/mod_io.debug.x64.dll` and `target/release/mod_io.dll` to `addons/modio/mod_io.x64.dll`.
  - Linux: Copy `target/debug/libmod_io.so` to `addons/modio/libmod_io.debug.x64.so` and `target/release/libmod_io.so` to `addons/modio/libmod_io.x64.so`.
